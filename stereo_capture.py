...
если точность калибровки плохая
   1. проверить что индексы камеры совпадают (хотябы по видео закрыть пальцем одну камеру и проверить какая картинка затемняется)
   2. проверить что камеры на одной высоте примерно (иначе не будут находится пары )
"""
